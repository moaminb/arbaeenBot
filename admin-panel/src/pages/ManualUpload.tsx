...
export default function ManualUpload() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!/^\+[0-9]+$/.test(phone)) {
      setStatus({ loading: false, message: 'لطفا شماره موبایل را همراه با کد کشور وارد کنید (مثال: +989123456789)', error: true })
      return
    }
        ...
        <div>
          ...
          <input 
            ...
            onChange={e => {
              ...
              if (val && !/^\+?[0-9]*$/.test(val)) {
                setStatus({ loading: false, message: 'لطفا فقط از اعداد انگلیسی و علامت + در شروع استفاده کنید.', error: true })
              } else {
